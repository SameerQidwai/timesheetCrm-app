...
const MonthlyTimesheet = ({navigation}) => {
  ...
  const getData = async () => {
    setFetching(true);
    let keys = {
      startDate: formatDate(sDate).startOf('month').format('DD-MM-YYYY'),
      endDate: formatDate(sDate).endOf('month').format('DD-MM-YYYY'),
      userId: appStorage['id'],
    };
    try {
      let {success, data, setToken} = await getTimesheetApi(
        keys,
  ...
  const actionTimeSheet = stage => {
    if (stage === 'Add') {
      navigation.navigate('Timesheet-Details', {
        sDate: formatDate(sDate).format('YYYY-MM-DD'),
      });
    } else {
